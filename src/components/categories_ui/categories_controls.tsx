...
export default function CategoriesControls({ searchTerm, setSearchTerm, viewMode, setViewMode, onAdd }: CategoriesControlsProps) {
  return (
    <div className="px-6 py-4 border-b flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
      <h2 className="text-xl font-semibold text-[#171D26]">All Categories</h2>
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 w-full lg:w-auto">
        ...
        <div className="flex bg-gray-100 rounded-lg p-1">
          <button onClick={() => setViewMode('list')} className={`px-3 py-1 rounded-md text-sm font-medium ${viewMode==='list'?'bg-white shadow-sm':'text-gray-600 hover:text-gray-900'}`}>List</button>
