...
export default function InventoryPage() {
  ...
  const canUpdate = canPerformAction("Inventory", "update");
  ...
  const handleEdit = (item: InventoryItem) => {
    ...
    if (!canUpdate) {
      toast.error("You don't have permission to update inventory items");
      return;
    }
      ...
      <Container>
        ...
        {showForm && canShowForm() ? (
          ...
          <div className="bg-white w-full flex items-center justify-between rounded-md border border-gray-200 border-b-0">
            ...
            {/* Only show Add Item button if user has create permission */}
            {canCreate && (
              <button
