...
export default function BulkUploadForm({
  ...
  function createEmptyRow(): CollectionRow {
    return {
      ...
      eligible: true,
      received: false,
      received_date: null, // 👈 Date field included
      given_by: "",
    };
  }
  ...
  return (
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 overflow-auto p-4">
      <form
        ...
        className="space-y-4 bg-white p-6 rounded-lg w-full max-w-7xl shadow-lg"
      >
        ...
        {rows.map((row) => (
          <div
            key={row.tempId}
            className="grid grid-cols-10 gap-2 items-center border p-3 rounded-lg bg-gray-50"
          >
            {/* Student */}
            <select
            ...
            {row.received ? (
              <input
                ...
                onChange={(e) =>
                  handleChange(row.tempId, "received_date", e.target.value)
                }
                className="p-2 border rounded"
                required={row.received}
              />
            ) : (
              <div className="text-gray-400 text-sm italic text-center">—</div>
            )}
