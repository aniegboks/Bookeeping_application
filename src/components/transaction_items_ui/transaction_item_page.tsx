...
import ViewTransactionModal from "@/components/transaction_items_ui/view_modal";
...
export default function InventoryTransactionsPage() {
    ...
    const canCreate = canPerformAction("Inventory Transactions", "create");
    const canUpdate = canPerformAction("Inventory Transactions", "update");
    const canDelete = canPerformAction("Inventory Transactions", "delete");
    ...
    const loadAllData = useCallback(async () => {
        ...
        try {
            ...
            try {
                const currentUserRes = await fetch("/api/get_current_user");
    ...
    const handleTransactionSubmit = async (
        ...
        if (editingTransaction && !canUpdate) {
            toast.error("You don't have permission to update transactions");
            return;
        }
        if (!editingTransaction && !canCreate) {
    ...
    const handleDistributionSubmit = async (data: CreateInventoryDistributionInput) => {
        ...
        // Check permission for distribution (using create permission)
        if (!canCreate) {
            toast.error("You don't have permission to create distributions");
            return;
        }
